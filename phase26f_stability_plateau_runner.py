...
PREREG = {
    "phase": "26F",
    "name": "Stability Plateau Sweep",
    "date_locked": "2025-10-08",
    "domains": ["bio", "synthetic", "cosmic"],
    "constants_locked": {
        "epsilon_set": np.round(np.arange(0.004, 0.018, 0.0003), 4).tolist(),
        "shock_set": [0.01, 0.02, 0.04],
        "agents_set": [1200, 1500, 2000],
        "epochs": 60000,
        "seeds": [1001, 1002, 1003, 1004, 1005, 1006],
    },
    "hypothesis_H1": "CCI surpasses 0.70 under low shocks & extended stability window while survival ≥0.80.",
    "null_H0": "CCI remains below 0.70 despite extended stability regime.",
}

STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTDIR = Path(f"./discovery_results/phase26f_stability_{STAMP}")
DATADIR = OUTDIR / "data"
...
(REPDIR / "phase26f_results.md").write_text(
    f"# Phase 26F — Stability Plateau Sweep ({STAMP})\nTargets: CCI≥0.70, survival≥0.80\nData: {runs_summary}\nRan: {count} runs\n"
)

print(f"[✓] Phase 26F complete → {OUTDIR}")
