...
from studies.trust_survival import adapter_trust_containment as adapter_cont
...
def run(config: dict[str, Any], seed: int = None):
    ...
    run_conf = {}
    run_conf["label"] = config.get("label", f"CL_{seed}")
    run_conf["mode"] = config.get("mode", prereg.get("mode", ["ARC_CASCADE_CL"])[0])
    ...
    try:
        adapter_cont._attach_base_trust(
            run_conf, cfg.get("controls", {}).get("base_noise", base_noise)
        )
    except Exception:
        pass

    if run_conf["mode"] in ["ARC_CASCADE", "ARC_CASCADE_CL"]:
        cfg.setdefault("resonance", {})
        cfg.setdefault("arc_controller", {})
        cfg.setdefault("cascade", {})
        cfg.setdefault("containment", {})
        try:
            adapter_cont._attach_arc_hooks(run_conf, cfg)
            # build edges if cascade
            if run_conf["mode"].startswith("ARC"):
                topo = prereg.get("supernet_topology", ["chain"])[0]
                edges = adapter_cont._build_supernet(topo, run_conf.get("clusters", 12))
                adapter_cont._attach_cascade(
                    run_conf,
                    cfg,
                    edges,
                    prereg.get("inter_kappa", [0.15])[0],
                    prereg.get("inter_delay", [10])[0],
                    prereg.get("seed_fraction", [0.25])[0],
                )
            if run_conf["mode"] == "ARC_CASCADE_CL":
                adapter_cont._attach_containment(run_conf, cfg)
        except Exception:
            pass
