"""Wrapper exposing run(config, seed) for Phase 9 automation.
Builds a single-run grid from the automation-provided config and calls
sweep_and_collect with a simcore.runner preference and local fallback.
...
def run(config: dict[str, Any], seed: int):
    ...
    run_cfg = {
        ...
        "epochs_cap": cfg.get("epochs", cfg.get("epochs_cap", 800)),
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "adapter_trust_env_feedback_wrapper: could not extract trajectories from sweep output"
            )
