...
def _attach_containment(run_cfg: dict[str, Any], cfg: dict[str, Any]):
    cl = cfg.get("containment", {})
    ...
    run_cfg["hooks"]["on_epoch_contain"] = {
        ...
        "args": {
            ...
            "absorption": cl.get("absorption", {}),
            "safety": {
                "cap_reservoir": cl.get("safety", {}).get("cap_reservoir", 1.5),
                "freeze_pulses": cl.get("safety", {}).get(
    ...
    run_cfg["hooks"]["on_epoch_release"] = {
        "name": "containment_reinject",
        "args": {
            "reinjection": cl.get("reinjection", {}),
            "keys": {
                "eps": "state.openness_eps",
                "reservoir": "state.reservoir",
                "lr": "traits.lr_trust",
...
def build_grid(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for mode in cfg.get("design", {}).get("factors", {}).get("mode", ["ARC_CASCADE"]):
        for topo in (
                        ...
                        for tau0 in (
                            ...
                            for g in (
                                ...
                                for shock in (
                                    ...
                                    if mode in ["ARC_CASCADE", "ARC_CASCADE_CL"]:
                                        _attach_arc_hooks(rcfg, cfg)
                                        _attach_cascade(rcfg, cfg, edges, k, d, sf)
                                    if mode == "ARC_CASCADE_CL":
                                        _attach_containment(rcfg, cfg)
                                    grid.append(rcfg)
    return grid
...
def run_study(study_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    save_exports(
        ...
        summary,
        figures=figs,
        study_name=study_cfg.get("name", "phase20_containment"),
    )
