"""Wrapper adapter exposing run(config, seed) for Phase 8 automation.
Builds a single-run grid and calls sweep_and_collect (preferring simcore.runner,
falling back to studies.trust_survival.adapter_trust).
...
def run(config: dict[str, Any], seed: int):
    ...
    run_cfg = {
        ...
        "agents": cfg.get("agents", study_cfg.get("design", {}).get("agents", 160)),
        "seeds": [seed],
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "adapter_trust_open_drift_wrapper: could not extract trajectories from sweep output"
            )
