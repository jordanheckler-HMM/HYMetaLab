"""Wrapper exposing run(config, seed) for Phase 10 automation.
Builds a single-run grid from the automation-provided config and calls
sweep_and_collect (preferring simcore.runner, falling back to local helpers).
Returns epoch-level trajectories filtered by seed.
"""
...
def run(config: dict[str, Any], seed: int):
    ...
    run_cfg = {
        ...
        "openness_eps": cfg.get(
            "openness_eps", cfg.get("openness", {}).get("eps", 0.1)
        ),
        "trust_tau0": cfg.get("trust_tau0", cfg.get("trust_tau", 0.5)),
        "epochs_cap": cfg.get("epochs", cfg.get("epochs_cap", 500)),
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "adapter_trust_adaptive_openness_wrapper: could not extract trajectories from sweep output"
            )
