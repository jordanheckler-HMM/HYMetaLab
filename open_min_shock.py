#!/usr/bin/env python3
"""
Shock-Recovery compact runner

Conditions: closed; agentio_eps0.0015_period40; agentio_eps0.0020_period40
Seeds: [101,202]; Horizon: 10000; Agents: 64->48; Log: 1->10
Shock: epoch 7000, single pulse: increase noise +15% on top-50% high-noise agents

Exports under outputs/open_min_shock/<timestamp>/
"""
import hashlib
import json
import math
...
OUT = Path(f"outputs/open_min_shock/{timestamp}")
...
def run_plan():
    for cond in PLAN:
        for seed in DEFAULTS["seeds"]:
            ...
            for epoch in range(HORIZON + 1):
                ...
                # shock at 7000 (single pulse)
                if epoch == 7000:
                    # top-50% agents get +15% noise => net increase = 0.15 * 0.5 of high-noise mass
                    delta = state["mass_high_noise"] * 0.15 * 0.5
                ...
                if len(hazard_hist) >= 50:
                    # collect last 50 CCI values safely (skip None)
                    ...
                    if len(cci_last50) >= 10:
                        slope = np.polyfit(np.arange(len(cci_last50)), cci_last50, 1)[0]
                        if (
                ...
                # logging (thinned)
                log_every = LOG1 if epoch <= EPOCHS_PHASE1 else LOG2
            ...
            # compute metrics: recovery times after shock
            # t_recover_CCI: first epoch >=7000 where CCI>=0.50
            t_recover_cci = None
            ...
            # AUH 7000-8000: sum hazard over epoch window (approximate integral)
            hazard_vals = [
                per_epoch_hazard[e]
                for e in range(7000, 8001)
                if per_epoch_hazard[e] is not None
            ]
            auh = float(np.sum(hazard_vals)) if hazard_vals else float("nan")
    ...
    # summary json: per-condition AUH mean + 95% CI (<=100 bootstraps)
    summary = {"timestamp": timestamp, "config": DEFAULTS, "results": {}}
    for c in PLAN:
        sub = runs_df[runs_df["label"] == c["label"]]
        if sub.empty:
            continue
        auh_vals = sub["AUH_7000_8000"].values
        ...
        summary["results"][c["label"]] = {
            "n_runs": len(sub),
            "AUH_mean": float(np.nanmean(auh_vals)),
            "AUH_ci": ci(auh_vals),
    ...
    # choose winner: lowest AUH, tie-break shortest max recovery; must have budgets_ok True for all runs
    winner = None
    winner_safe = False
    reason = ""
    cand = []
    for k, v in summary["results"].items():
        ...
        if sub.empty:
            continue
        if int(v["budgets_ok_count"]) != len(sub):
            continue
        cand.append((k, v["AUH_mean"], v))
    if cand:
        cand_sorted = sorted(cand, key=lambda x: (x[1],))
        # tie-break: compare max recovery times
        best_label = cand_sorted[0][0]
        # compute tie-break among equals
        min_val = cand_sorted[0][1]
        ties = [c for c in cand_sorted if abs(c[1] - min_val) < 1e-12]
        if len(ties) > 1:
            best = None
            best_recovery = 10**9
            for t in ties:
                ...
                # compute worst-case recovery (max over seeds of max(t_recover_CCI,t_recover_hazard))
                recs = [
                    (
                        (x if not math.isnan(x) else HORIZON + 1)
                        if x is not None
                        else HORIZON + 1
                    )
                    for x in sub["t_recover_CCI"]
                ]
                recs2 = [
                    (
                        (x if not math.isnan(x) else HORIZON + 1)
                    ...
                    for x in sub["t_recover_hazard"]
                ]
                worst = max([max(a, b) for a, b in zip(recs, recs2)])
                if worst < best_recovery:
                    best_recovery = worst
                    best = lbl
            winner = best
        else:
        ...
        # no budget-safe candidate; pick lowest AUH (even if budgets exhausted) but flag NOT SAFE
        all_cands = []
        for k, v in summary["results"].items():
            all_cands.append((k, v["AUH_mean"]))
        if all_cands:
    ...
    if not traj_df.empty:
        ...
        plt.axvline(7000, color="k", linestyle="--", label="shock")
    ...
    if not traj_df.empty:
        ...
        plt.axvline(7000, color="k", linestyle="--", label="shock")
        plt.legend(fontsize=8)
        plt.title("hazard shock window")
        plt.savefig(OUT / "figures" / "hazard_long.png")
        plt.close()

    # report
    md = f"Shock-Recovery results ({timestamp})\n\nPLAN: {[p['label'] for p in PLAN]}\n\nWinner: {winner} (production_safe={winner_safe})\n\n"
    md += "Per-condition summary:\n"
    for k, v in summary["results"].items():
        md += f"- {k}: AUH_mean={v['AUH_mean']:.6f}, AUH_ci={v['AUH_ci']}, budgets_ok_count={v['budgets_ok_count']}\n"
    md += "\nMetrics per run in data/runs_summary.csv\n"
    with open(OUT / "report" / "open_min_shock_results.md", "w") as f:
        f.write(md)
    ...
    with open(OUT / "SHA256SUMS.txt", "w") as f:
        for k, v in sorted(checks.items()):
            f.write(f"{v}  {k}\n")
    bundle_name = f"open_min_shock_{timestamp}.zip"
