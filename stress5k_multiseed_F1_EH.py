#!/usr/bin/env python3
"""
Run 6 seeds of F1_EH_early_periodic to 5000 epochs, compute bootstrap CIs.
Optimized logging to keep runtime short (target < 1 min, max 10 min).
"""
import hashlib
import json
import os
...
OUT = Path(f"outputs/stress5k_multiseed/{timestamp}")
for d in ["data", "figures", "report", "bundle", "logs"]:
    (OUT / d).mkdir(parents=True, exist_ok=True)

TIME_BUDGET_SEC = 600  # 10 min max guard
SEEDS = [1, 2, 3, 4, 5, 6]
EPOCHS = 5000
LOG_EVERY = 20  # tuned for speed
AGENTS = 48
...
def simulate_seed(seed, epochs_cap=EPOCHS, log_every=LOG_EVERY, agents=AGENTS):
    ...
    for epoch in range(epochs_cap):
        ...
        expansion_boost = 1.0
        hygiene_boost = 1.0
        # fixed early expansion (matches prior run)
        ...
        if epoch % log_every == 0 or epoch == epochs_cap - 1:
            traj.append(
                {
                    "run_id": f"F1_EH_early_periodic_5000_s{seed}",
                    "seed": seed,
                    "epoch": epoch,
                    "CCI": cci,
                    "survival": survival,
        ...
        # early stop condition (aggressive)
        if risk >= 0.6:
            break
...
# bootstrap CIs (n=2000 fast resamples) on final_CCI and stability
...
plt.title("F1_EH_early_periodic 6-seed CCI")
plt.savefig(OUT / "figures" / "cci_6seed.png")
plt.close()
...
bundle_name = f"stress5k_multiseed_{timestamp}.zip"
...
with open(OUT / "report" / "readme.md", "w") as f:
    f.write(
        f"6-seed run for F1_EH_early_periodic_5000\nTimestamp: {timestamp}\n\nBootstrap summary:\n{json.dumps(bootstrap,indent=2)}\n\nOutputs: {OUT}\n"
    )
