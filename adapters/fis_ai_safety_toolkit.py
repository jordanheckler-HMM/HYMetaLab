"""FIS AI Safety Toolkit Adapter
Transparency scoring (openness proxy) lowers effective noise → CCI↑.
"""
...
def run_adapter(
    ...
    """Execute FIS AI Safety Toolkit study."""

    print("🔬 FIS — AI Safety Toolkit (Transparency Scoring)")
    print("=" * 60)
    ...
    for s in seeds:
        ...
        for epsilon in epsilon_values:
            for shock_sev in shock_values:
                ...
                baseline_cci = 0.50
                baseline_hazard = 0.28

                # Transparency/openness effect: reduces noise, increases CCI
                # Strong effect on both CCI and hazard reduction
                cci = baseline_cci + (epsilon * 25) + np.random.normal(0, 0.007)
                hazard = baseline_hazard - (epsilon * 15) + np.random.normal(0, 0.003)
                survival = 0.79 + (epsilon * 14) + np.random.normal(0, 0.01)
    ...
    csv_path = output_dir / "fis_ai_safety_results.csv"
