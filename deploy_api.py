#!/usr/bin/env python3
"""
API Deployment Script for Reality Loop Website
Deploys real-time metrics API endpoint
"""
...
class APIDeployer:
    ...
    def deploy_metrics_api(self):
        """Deploy real-time metrics API"""
        print(f"🚀 Deploying metrics API...")
        ...
        # Create metrics API module
        api_content = self.generate_metrics_api_code()
        api_file = api_dir / 'metrics_api.py'
        
        with open(api_file, 'w') as f:
            f.write(api_content)
        
        # Create API configuration
        config_content = self.generate_api_config()
        config_file = api_dir / 'config.json'
        
        with open(config_file, 'w') as f:
            json.dump(config_content, f, indent=2)
        
        print(f"✅ Metrics API deployed: {api_file}")
        return api_file
    
    def generate_metrics_api_code(self):
        """Generate metrics API Python code"""
...
'''
    ...
    def generate_api_config(self):
        ...
        return {
            ...
            'endpoints': [
                '/api/metrics',
                '/api/metrics/guardian',
                '/api/metrics/truthlens', 
                '/api/metrics/cci',
                '/api/health'
            ],
            'cors_enabled': True,
...
def main():
    parser = argparse.ArgumentParser(description='Deploy Reality Loop API')
    parser.add_argument('--module', required=True, help='API module name')
    parser.add_argument('--params', help='JSON parameters for API configuration')
    
    args = parser.parse_args()
    
    print("🚀 Reality Loop API Deployment")
    print("=" * 30)
    
    try:
        deployer = APIDeployer(args.module, args.params)
        api_file = deployer.deploy_metrics_api()
        
        print(f"\n🎉 API deployment complete")
        print(f"   File: {api_file}")
        print(f"   Endpoints: /api/metrics, /api/health")
        return 0
            
    except Exception as e:
        print(f"\n❌ API deployment error: {e}")
        return 1
